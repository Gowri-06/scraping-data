...
def product_hunt(): 
    ...
    query = """
      query {
        posts(postedAfter: "2023-01-01", postedBefore: "2023-01-31", order: RANKING,after: "MTgw" ) {
          edges {
            node {
              id
              name
    ...
    # df.to_excel("ProducthuntSecondset.xlsx", index=False)
    # print(df)
    df_old = pd.read_excel('ProducthuntSecondset.xlsx')
    df_new = pd.concat([df_old,df], ignore_index=True)
    df_new.to_excel('ProducthuntSecondset.xlsx', index=False)
...
def processing_all_data():
    ...
    df = pd.read_excel("ProducthuntSecondset.xlsx",sheet_name="Sheet1") 
    # print(df)
    url = df["Url"]
    id_list= df["Id"]
    ...
    for i in range(0,100):
        ...
        try:
           for group_maker in var:
            ...
            dict2["Desgination"] = ""   
            dict2["Hashtag_value"] = "" 
            dict2["Followers_value"] = ""  
            dict2["Following_value"] = ""   
            dict2["Twitter Url"] =  ""
            dict2["Linkedin Url"] = ""
...
# read the existing Excel file
    df_existing = pd.read_excel('ProducthuntSecondset.xlsx')
    df_existing["Makers"] = list2_list2
    df_existing.to_excel('ProducthuntSecondset.xlsx', index=False)
  


    print("list2_list2",list2_list2)
    print("list2_list2***len",len(list2_list2))
